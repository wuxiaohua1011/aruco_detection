...
class ARMarkerLocalization:
    def __init__(self, json_qr_code_file_path: str, intrinsics_matrix, distortion_coefficient, visualize=True):
        self.json_in_path: Path = Path(json_qr_code_file_path)
    ...
    def get_position_list(self, gray):  # Get list of all global positions based on each ar tag in the frame
        """
        ...
        if np.all(ids != None):
            ...
            if self.visualize:
                image = aruco.drawDetectedMarkers(gray, corners=corners)
                for i, val in enumerate(ids):
                    image = aruco.drawAxis(image, self.intrinsics_matrix, distCoeffs=self.distortion_coefficient,
                                           rvec=rvecs[i], tvec=tvecs[i], length=0.1)

                visualize("detected markers", image)
    ...
    def compute3D(self, matches, cur_kp, prev_kp, cur_depth):
        ...
        return (np.linalg.pinv(self.intrinsics_matrix) @ cur_2d @ current_depth).T, (
                self.intrinsics_matrix @ prev_2d @ previous_depth).T
...
def sort_dir(dirpath) -> List[Path]:
    import os
    from pathlib import Path
...
def prep_path(dir_path) -> Tuple[List[Path], List[Path], List[Path]]:
    front_rgb_paths: List[Path] = sort_dir(dir_path / "front_rgb")
    front_depth_paths: List[Path] = sort_dir(dir_path / "front_depth")
    transform_paths: List[Path] = sort_dir(dir_path / "transform")
    min_num_files = min(len(front_rgb_paths), len(front_depth_paths), len(transform_paths))

    front_rgb_paths = front_rgb_paths[:min_num_files]
    front_depth_paths = front_depth_paths[:min_num_files]
    transform_paths = transform_paths[:min_num_files]

    return front_rgb_paths, front_depth_paths, transform_paths


def read_transform_file(file_path: str):
...
def visualize(window_name, image):
...
if __name__ == "__main__":
    # [ 640x480  p[317.769 240.86]  f[382.484 382.484]  Brown Conrady [0 0 0 0 0] ]
    pass
    archive_path = Path("archive")
    fast_output_folder_path = archive_path / "fast_output" / "output"
    slow_output_folder_path = archive_path / "slow_video" / "output"
    front_rgb_paths, front_depth_paths, transform_paths = prep_path(slow_output_folder_path)
    json_qr_code_file_path: Path = Path("track_1.json")
    ...
    distortion_coefficient = np.array([0, 0, 0, 0, 0]).astype(float)

    ar_marker_localization = ARMarkerLocalization(json_qr_code_file_path=json_qr_code_file_path.as_posix(),
                                                  intrinsics_matrix=intrinsics_matrix,
                                                  distortion_coefficient=distortion_coefficient,
                                                  visualize=True)

    for i in range(len(front_rgb_paths)):
        rgb_image = cv2.imread(front_rgb_paths[i].as_posix())
        depth_image = np.load(front_depth_paths[i].as_posix())
        x, y, z, roll, pitch, yaw = read_transform_file(transform_paths[i].as_posix())
        should_run = ar_marker_localization.run(img_arr=rgb_image, depth_arr=depth_image)
